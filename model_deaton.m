% Matlab class to implement the Deaton's model of consumption and savings
...
properties (Access=public)
	...
	theta		= 1 		; %CRRA coefficient (log utility if ==1)
	inc0		= 0.75	; %income equation: constant
	inc1		= 0.04	; %income equation: age coef
	inc2		= 0.0004;	%income equation: age^2 coef
...
methods (Access=public)
    ...
    % for it=[FILL IN]:-1:[FILL IN]
        ...
        % cons0=me.imutil([FILL IN]*[FILL IN]);%current period consumption
        ...
        if compute_value
	        ...
	        me.value(it)=me.value(it).inset(0,ev(1),0);%last zero to insert first point
	      end
	...
	function solve_vfi(me)
		...
		function res=bellman(it,wealth,consumption)
			...
			if consumption<wealth
				...
				% res=me.util(consumption)+[FILL IN];
    ...
    for it=1:me.Tbar
    	if it==1
    		%draw initial wealth form uniform distribution on given interval
				...
				me.sims.income(:,it)=me.income(it,me.sims.shock(:,it));
			end
			%choice
	...
	function ax=plot(me,what2plot)
		%plot computed entities: policy, value, "sims field_name"
		if nargin<2
			what2plot='solution';
		end
